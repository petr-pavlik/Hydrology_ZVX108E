...
lapply(
  X = setdiff(
    c("whitebox", "terra", "sf", "ggplot2", "tidyterra"), 
    installed.packages()[, "Package"]), 
  FUN = install.packages)
```

After the successful installation, the `library` function load the namespace of the pacakges and allows their exported functions to be used directly.
...
```{r, collapse=TRUE}
install_whitebox("~/Desktop/GIS/") #<>
wbt_init(exe_path = "~/Downloads/WBT/whitebox_tools") # <5>
