...
1. Find the mean, median, mode and standard deviation of the following series. 
 ...
 **1.** Which of the following is functions is used to filter rows? `r mcq(opts_2)`; 
 which operates on columns? `r mcq(opts_2)`.   
 **2.** `r mcq(c("Alec Guinness", answer = "Mark Hamill", "Harrison Ford"))`     
 **3.** The difference between operators `%>%` and `|>` lies in `r longmcq(opts_4)`.     
 **4.** Hi my name is - 2 + 2 is `r torf(T)`.    
 **5.** What is the primary function of the `tidyr` package? `r longmcq(opts_6)`.    
